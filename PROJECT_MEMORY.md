...
> Last updated: 2026-08-24 (README v2 + charts done; HF repos PUBLIC + anonymous pull verified; Space deploy pending HF token)
>
...
8. RESOLVED (2026-08-24): HF repos flipped PUBLIC; anonymous (token-less) pull + model-card
   snippet verified working.
9. RESOLVED (2026-08-24): README v2 written (journey v1.0→v2.0, mermaid architecture,
   honest-numbers table, use cases, gaps/limitations) + 3 chart PNGs in docs/assets/
   (version_progress, risk_coverage, gold_subset — generated by src/experiments/make_readme_charts.py).
10. PAUSED (2026-08-24): HF Space deploy blocked by HF policy — Docker (and Gradio) Spaces on
    free cpu-basic now require PRO ($9/mo); only Static Spaces are free (can't run FastAPI).
    create_repo returned 402. EXACT STEPS when resuming (5-min job, everything else is ready):
    a. Subscribe to HF PRO (or pick an alternative host with >=1 GB free RAM).
    b. Fix already applied: apps/webapp/SPACE_README.md rewritten clean UTF-8 (was mojibake).
    c. python: HfApi(token=HF_TOKEN).create_repo('sandeep1103/lilly-fashion-muse',
       repo_type='space', space_sdk='docker', private=False, exist_ok=True)
    d. Stage a folder with: Dockerfile (apps/webapp/Dockerfile -> root), README.md
       (= SPACE_README.md), backend/, frontend/, requirements.txt, src/sentence_splitter.py
       (Dockerfile expects repo-root layout: apps/webapp/... + src/sentence_splitter.py).
    e. upload_folder(repo_type='space') -> build runs on the Hub (torch CPU install, ~5-10 min).
    f. Models are PUBLIC now, so no HF_TOKEN secret needed at boot (anonymous pull verified);
       add one anyway if rate limits bite.
    g. Verify: GET https://sandeep1103-lilly-fashion-muse.hf.space/api/health + regression trio
       (clear-pos 10.0+, clear-neg 1.6-, "beautiful pattern but runs extremely small" -> UNSURE @0.55).
    h. Rotate/delete the HF token shared in chat (Sandeep said he will).
11. Git hygiene (2026-08-24): models/ (7.7 GB checkpoints) and int8_state_dict.pt (132 MB,
    over GitHub cap) gitignored — weights live on the HF Hub; code + results + data/processed
    CSVs (~11 MB) + charts are committed instead.
