...
10. BLOCKED (2026-08-24): HF Space deploy needs an HF_TOKEN (fine-grained, read) set as a
    Space secret to pull the (now public) models at boot; no token available in this shell —
    manual step for Sandeep: create Space (sdk: docker, app_port 7860), push apps/webapp/ +
    Dockerfile + SPACE_README.md, set HF_TOKEN secret, verify cold boot.
